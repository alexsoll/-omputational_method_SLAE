...
class slae_gui:
    ...
    def Run(self,master):
        ...
        label_gauss = Label(master, text = ("Gauss time = " + str(round(time_methods[0] * 1000, 6)) + ' ms'))
        label_gauss.place(x = 125 ,y = (self.size  + 6)* int(self.ent.winfo_height()) + 10)
        label_kramer = Label(master, text = ("Kramer time = " + str(round(time_methods[1] * 1000, 5)) + ' ms'))
        label_kramer.place(x = 125 ,y = (self.size  + 7)* int(self.ent.winfo_height()) + 10)
        label_lu = Label(master, text = ("LU methods time = " + str(round(time_methods[2] * 1000, 5)) + ' ms'))
        label_lu.place(x = 125 ,y = (self.size  + 8)* int(self.ent.winfo_height()) + 10)
        label_seidel = Label(master, text = ("Seidel time = " + str(round(time_methods[3] * 1000, 5)) + ' ms'))
        label_seidel.place(x = 125 ,y = (self.size  + 9)* int(self.ent.winfo_height()) + 10)
        label_simple = Label(master, text = ("Simple iterations time = " + str(round(time_methods[4] * 1000, 5)) + ' ms'))
